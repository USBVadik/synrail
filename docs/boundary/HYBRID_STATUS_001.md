...
Hybrid is not promoted because the current measured evidence is still mixed:

- `evidence_count = 2`
- current verdict mix = `UNCLEAR`, `SYNRAIL_BETTER`

That is enough to show hybrid is not empty policy theater.

It is still not enough to treat hybrid as a settled default middle lane.

## Why it is not fully demoted

Hybrid is not fully demoted because the current evidence now includes:
...
- hybrid has one stronger win
- but the class-level signal is still not clean enough for promotion
...
3. keep hybrid bounded and secondary until the class-level measured signal stops being mixed
...
- only promote hybrid if the measured class-level signal stops staying unclear
