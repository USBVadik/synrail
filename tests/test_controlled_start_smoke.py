...
class ControlledStartSmokeTests(unittest.TestCase):
    ...
    def test_start_creates_bootstrap_and_proof_request(self) -> None:
        with tempfile.TemporaryDirectory(prefix="synrail_controlled_start_") as tmpdir:
            ...
            self.assertIn("Do not touch it on the happy path", start.stdout)
            ...
            self.assertIn("create readback with synrail readback-template > .synrail/readback.txt", start.stdout)
            self.assertIn("scenario proof with synrail scenario-proof-template > .synrail/scenario_proof.txt", start.stdout)
            self.assertIn("synrail readback-template", start.stdout)
            self.assertIn("synrail scenario-proof-template", start.stdout)
    ...
    def test_check_without_final_result_uses_proof_request_guidance(self) -> None:
        with tempfile.TemporaryDirectory(prefix="synrail_proof_request_") as tmpdir:
            ...
            self.assertIn("fallback-only surface", check.stdout)
            self.assertIn("unless synrail check later asks for readback", check.stdout)
            self.assertIn("unless synrail check later asks for scenario proof", check.stdout)
