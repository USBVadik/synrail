...
| `synrail bug-packet` | You need one compact failure packet for debugging or issue filing without assembling state/report/doctor/packet truth by hand | one `artifact-root` with at least `state.json` and `report.json`, plus any available doctor/packet/observability/thin-output artifacts | one machine-readable bug packet plus one issue-ready markdown body | hosted telemetry, automatic upload, sensitive data capture, domain correctness by itself |
...
For the concrete installable contour, read [ALPHA_LANE_001.md](../../docs/core/ALPHA_LANE_001.md) and inspect [alpha_shell_run_008](../../fixtures/alpha_shell_run_008).
