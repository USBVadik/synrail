    ...
    *,
    ...
    if args.mode == "dev":
        ...
        if code == 0 and root:
            ...
            criteria_completed = context.write_acceptance_criteria(root, generated_by="synrail init")
    ...
    if root:
        ...
        context.print_init_summary(root=root, state_file=Path(args.output))
    return 0
...
def cmd_start(
    ...
    *,
    ...
    if existing_state_path.exists():
        try:
            existing_state = context.load_json(existing_state_path)
    ...
    if not getattr(args, "run_id", None):
        args.run_id = context.default_alpha_run_id()
    if not getattr(args, "task_class", None):
        args.task_class = existing_state.get("task_class", context.default_task_class) if existing_state else context.default_task_class

    task_identity, prompt_identity = context.resolve_start_identities(args, root=root)
    if not task_identity:
        if args.mode == "dev":
            ...
            print(
                "What to do next: run `"
                + context.plain_shell_command("start", "Describe the bounded local change.", project_root=project_root)
                + "` or pass --task-identity, then retry."
            )
        return 2

    existing_proof = context.existing_preferred_proof_artifacts(root)
    previous_state = existing_state.get("state", "") if existing_state else ""
    active_bootstrap = context.alpha_file(root, "bootstrap").exists()
    if existing_state and active_bootstrap and previous_state and previous_state not in context.terminal_run_states and not existing_proof:
        if args.mode == "dev":
            print(
                json.dumps(
                    {
                        ...
                        "next_command": context.shell_command(root, "check", project_root=project_root),
                    },
                    ensure_ascii=True,
                )
            )
        else:
            context.print_existing_run_summary(root=root, state_file=existing_state_path, project_root=project_root)
        return 0
    if existing_proof:
        if previous_state in ("CLOSURE_ACCEPTED", "CLOSURE_REJECTED"):
            for _aid, path in context.preferred_proof_artifact_paths(root).items():
                path.unlink(missing_ok=True)
            proof_request_path = context.alpha_file(root, "proof_request")
    ...
    criteria_completed = context.write_acceptance_criteria(root, generated_by="synrail start")
    if criteria_completed.returncode != 0:
        return _print_completed_failure(criteria_completed)
    validation = context.apply_bootstrap_defaults(args, root=root)
    if validation:
        context.save_bootstrap_json(context.alpha_file(root, "bootstrap_validation"), validation)
    context.update_last_known_final_result_hash(
        Path(args.output),
...
def cmd_refresh_acceptance(
    ...
    *,
    ...
    if not root:
        ...
        return 2
    profile = context.alpha_file(root, "project_profile")
    if not profile.exists():
        ...
        return 2
    completed = context.write_acceptance_criteria(root, generated_by="synrail refresh-acceptance")
    if completed.returncode != 0:
        return _print_completed_failure(completed)
    state_file = context.alpha_file(root, "state")
    if state_file.exists():
        validation_completed = context.write_acceptance_validation(root, criteria_file=context.alpha_file(root, "acceptance_criteria"), state_file=state_file)
        if validation_completed.returncode != 0:
            return _print_completed_failure(validation_completed)
    if args.mode == "dev":
        print(json.dumps(
            {
                "criteria": context.load_json(context.alpha_file(root, "acceptance_criteria")),
                "validation": context.load_json(context.alpha_file(root, "acceptance_validation")) if context.alpha_file(root, "acceptance_validation").exists() else None,
            },
            indent=2,
            ensure_ascii=True,
        ))
    else:
        context.print_acceptance_refresh_summary(root=root)
    return 0
